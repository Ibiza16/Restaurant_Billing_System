...
root.title("Bite & Sip")      # Title is set
...
def print_receipt_pdf():
    ...
    file_name = f"{Receipt_Ref.get()}.pdf"
    ...
    c.drawString(200, y, "Bite & Sip")
    y -= 40

    c.setFont("Helvetica", 12)
    c.drawString(50, y, f"Receipt Ref: {Receipt_Ref.get()}      Date: {DateofOrder.get()}")
    y -= 30

    c.drawString(50, y, "Items")
    c.drawString(300, y, "Cost of Items")
    y -= 20

    def draw_line(item, value):
        nonlocal y
        c.drawString(50, y, f"{item}")
        c.drawString(300, y, f"{value}")
        y -= 20

    draw_line("Fries", E_Fries.get())
    draw_line("Lunch Meal", E_Lunch.get())
    draw_line("Burger", E_Burger.get())
    draw_line("Pizza", E_Pizza.get())
    draw_line("Cheese Burger", E_CheeseBurger.get())
    draw_line("Mac Nuggets", E_MacNuggets.get())
    draw_line("Mac Puff", E_MacPuff.get())
    draw_line("Chicken Wings", E_ChickenWings.get())
    draw_line("Coffee Cake", E_Coffee_Cake.get())
    draw_line("Red Velvet Cake", E_Red_Velvet_Cake.get())
    draw_line("Black Forest Cake", E_Black_Forest.get())
    draw_line("Boston Cream Cake", E_Boston_Cream_Cake.get())
    draw_line("Latte", E_Latta.get())
    draw_line("Coke", E_Coke.get())
    draw_line("Pepsi", E_Pepsi.get())
    draw_line("Cappuccino", E_Cappuccino.get())

    y -= 20
    c.drawString(50, y, f"Cost of Food: {CostofFood.get()}")
    y -= 20
    c.drawString(50, y, f"Tax Paid: {PaidTax.get()}")
    y -= 20
    c.drawString(50, y, f"Cost of Cakes and Drinks: {CostofCakesandDrinks.get()}")
    y -= 20
    c.drawString(50, y, f"Sub Total: {SubTotal.get()}")
    y -= 20
    c.drawString(50, y, f"Service Charge: {ServiceCharge.get()}")
    y -= 20
    c.drawString(50, y, f"Total Cost: {TotalCost.get()}")
...
btnPrint = Button(fb2, padx=16, pady=1, bd=4, fg="black", font=('arial', 16, 'bold'), width=5,
                  text="Print", command=print_receipt_pdf).grid(row=1, column=0, columnspan=4, sticky=W+E+N+S)
